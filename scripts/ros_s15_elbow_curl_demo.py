...
def require_safe_args(args: argparse.Namespace) -> None:
    ...
    if args.waypoint_dwell < 0.2:
        raise SystemExit("--waypoint-dwell must be >= 0.2s for the slow elbow-curl demo.")
...
def run_hand_sequence(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(f"sending_{args.side}_hand_open={open_pose}")
...
def main() -> int:
    ...
    try:
        ...
        waypoints = build_waypoints(starts, targets, indices, [arm], args.max_step_deg)
        ...
        return_waypoints = build_waypoints(targets, {arm: starts[arm]}, indices, [arm], args.max_step_deg)

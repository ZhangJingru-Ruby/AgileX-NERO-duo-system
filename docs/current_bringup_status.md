...
| S14 末端执行器 | Active; left-hand independent CAN bench branch started | Both dexterous hands are mechanically installed and stable. Arm A = right hand, Arm B = left hand. S14.1 no-motion snapshot `20260629_074337` is accepted for communication/read-only health. Web screenshots show current end-effector config `强脑灵巧手`, hand page `普通灵巧手`, model `revo2`, mode `位置控制`, and enable without error, but Web single-finger send produced no hand motion. New Drive docs reviewed in `docs/s14_linker_drive_review.md` confirm LinkerHand L6 and introduce a Linker/LBOT controller stack at `192.168.10.21`; the live network probe did not reach that IP. Photos under `docs/pics/灵巧手连接设备/` show a bench DC supply plus USB-CAN debug hardware, not an IP controller. Latest wiring review says the NERO J6 installed topology and the LinkerHand direct bench-test topology are mutually exclusive. Operator has now disconnected the left hand from the arm/J6 and powered it independently; `灵巧手左手上电操控示意图.jpeg` shows about `24.00 V` and `0.122 A`. Live host checks found no serial device and instead found PEAK-System `XCAN-USB` on SocketCAN `can1`, USB path `1-3.4.4:1.0`. Next gate: follow `docs/s14_left_hand_can_bringup_plan.md`, confirm `can1` is UP at `1000000`, and run `scripts/s14_linkerhand_l6_can_readonly_probe.sh can1 left`. No finger motion is authorized before read-only acceptance. |

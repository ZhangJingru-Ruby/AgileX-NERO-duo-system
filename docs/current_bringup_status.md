...
| S12 控制隔离与日志闭环 | S12.1 Arm A accepted; S12.2 Arm B pending | Arm A `joint1 +30 deg` executed and returned. Operator observed the motion direction matched `lab_world -Y`; Arm B did not visibly move. Script reported max passive deviation `0.005 deg`, target reach error about `0.619 deg`, return error about `0.595 deg`, and A/B final `err_status: 0`. Post-motion snapshot `20260626_080809` is clean: failed captures `0`, A/B about `200 Hz`, A/B `err_status: 0`, no joint-limit flags, and no joint-communication flags. |

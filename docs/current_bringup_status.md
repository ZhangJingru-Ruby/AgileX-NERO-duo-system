...
| S11 双臂实验基线 | Partially measured; read-only ROS validation blocked | `lab_world` is defined with Arm A center as origin and `+X` from Arm A to Arm B. Arm B translation is recorded as `x=0.260 m, y=0, z=0`. Arm B yaw candidate is `pi` from Web-axis observation and requires RViz validation. On 2026-06-26 the dual read-only launch reached Arm B feedback but Arm A failed to get firmware version, so Arm A CAN read-only communication must be restored before TF validation. |

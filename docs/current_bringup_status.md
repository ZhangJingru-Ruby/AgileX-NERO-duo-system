...
| S12 控制隔离与日志闭环 | S12.1 Arm A dry-run accepted; execution pending | Plan `docs/s12_control_isolation_plan.md` defines target-arm control plus passive-arm read-only monitoring. Arm A `joint1 +30 deg` dry-run changed only Arm A `joint1` from `1.109 deg` to `31.109 deg`; Arm B was passive read-only. Both target and passive statuses had `err_status: 0`. |

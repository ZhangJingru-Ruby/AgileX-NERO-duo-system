...
| S13 低风险双臂协同原语 | First execution numeric pass; world-direction semantics failed; post-motion snapshot accepted | Arm A `joint1 +30 deg` and Arm B `joint1 -30 deg` executed and returned with A/B final `err_status: 0`; non-target joint deviations stayed small (`arm_a: 0.013 deg`, `arm_b: 0.008 deg`). Operator observed Arm B moved in the same visible direction as Arm A instead of the expected opposite direction, so this cannot be accepted as the intended S13 primitive. Post-motion snapshot `20260626_090214` is clean: failed captures `0`, A/B about `200 Hz`, A/B `err_status: 0`, no joint-limit flags, and no joint-communication flags. Next gate: corrected J1 direction-sign dry-run. |

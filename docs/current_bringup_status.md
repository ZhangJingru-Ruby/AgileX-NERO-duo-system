...
| S15 双臂双手协调脚本 | Implemented; left-side dry-run next | New scripts `launch_s15_dual_arm_hand_observe.sh` and `ros_s15_arm_hand_sequence.py` provide RViz observation, selectable read-only/active ROS arm drivers, static TF, segmented arm motion, LinkerHand SDK hand open/close/open, conservative posture guards, dry-run default, and explicit execution confirmations. On 2026-07-02 raw RViz showed horizontal arms while the real arms were hanging. Diagnostics showed live feedback subscribers and accepted S11 root TF were present, so an RViz-only S15 visual anchor was added at `/arm_a/visual/joint_states` and `/arm_b/visual/joint_states`. The operator then confirmed anchored RViz robot positions and visual following are correct. Visual topics remain observation-only, not control/planning/calibration. Next gate: S15 `--side left` dry-run with the observation session still read-only. |

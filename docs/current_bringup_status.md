...
| S15 双臂双手协调脚本 | Gesture redesign in progress; return-to-initial added | New scripts `launch_s15_dual_arm_hand_observe.sh`, `ros_s15_arm_hand_sequence.py`, and `ros_s15_return_to_initial.py` provide RViz observation, selectable read-only/active ROS arm drivers, static TF, segmented arm motion, LinkerHand SDK hand open/close/open, conservative posture guards, dry-run default, and explicit execution confirmations. On 2026-07-02 raw RViz showed horizontal arms while the real arms were hanging; the anchored RViz visual path was added and operator accepted visual pose/follow. The `--side left` dry-run maps to Arm B + left hand `can1`, generated 9 waypoints, and passed with max planned joint delta `89.868 deg`. First left execute published waypoint 1 but Arm B feedback stayed at the start values and timed out, then the script called A/B emergency stop before any hand command. Follow-up diagnostics confirmed active drivers are running with `auto_enable=True`, `control_enabled=True`, `speed_percent=5`, and normal arm status. The Arm B J1 `+2 deg` command-path probe using the accepted S12 isolation script passed by operator report. A later attempt showed the absolute J1/J2/J3 target did not match the intended human-like elbow-curl gesture. Next gate: use `ros_s15_return_to_initial.py` to return to S15 park, then run small joint-mapping probes for Arm B J2/J3/J4 before defining the elbow-curl/fist gesture. |

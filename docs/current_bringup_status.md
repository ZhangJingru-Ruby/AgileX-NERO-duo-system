...
| S13 低风险双臂协同原语 | Dry-run and hold accepted; execution pending | Dual-active driver state is stable enough for the first gate. Dry-run target changes only Arm A `joint1` from `1.111 deg` to `31.111 deg` and Arm B `joint1` from `-1.988 deg` to `-31.988 deg`; A/B `err_status: 0`; `hold_max_dev_deg` is `0.0` for both arms. |

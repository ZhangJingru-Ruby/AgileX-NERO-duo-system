...
| Planned end effector state | S14.8C left LinkerHand SDK open-anchor execute pending; no accepted finger actuation yet |
...
The open-anchor dry-run is accepted; see
`docs/s14_left_hand_open_anchor_dry_run_result_20260630.md`.

The immediate next step is the explicit open-anchor execute gate:
...
This sends one SDK `open` preset command. Full SDK demos, GUI, gesture loops,
`get_state()`, and `get_torque()` remain blocked until the first-motion gate is
accepted.

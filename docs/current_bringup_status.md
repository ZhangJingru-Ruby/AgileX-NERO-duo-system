...
| S11 双臂实验基线 | Partially measured; TF validation in progress | `lab_world` is defined with Arm A center as origin and `+X` from Arm A to Arm B. Arm B translation is recorded as `x=0.260 m, y=0, z=0`. Arm B yaw candidate is `pi` from Web-axis observation and requires RViz validation. After the earlier Arm A read-only launch blocker, the operator reports terminal 1 is now running normally. Terminal 2 static TF failed due to command-line newline splitting; use `scripts/publish_s11_static_tf_candidate.sh`. |

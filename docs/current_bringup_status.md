...
| S15 双臂双手协调脚本 | Slow elbow-curl probe ready | New scripts `launch_s15_dual_arm_hand_observe.sh`, `ros_s15_arm_hand_sequence.py`, `ros_s15_return_to_initial.py`, and `ros_s15_elbow_curl_demo.py` provide RViz observation, selectable read-only/active ROS arm drivers, static TF, segmented arm motion, LinkerHand SDK hand open/close/open, conservative posture guards, dry-run default, and explicit execution confirmations. On 2026-07-02 raw RViz showed horizontal arms while the real arms were hanging; the anchored RViz visual path was added and operator accepted visual pose/follow. The first absolute J1/J2/J3 target did not match the intended elbow-curl gesture. Operator then returned to the initial posture successfully and found by Web control that left-side Arm B `J1 -10 deg` plus `J4 +10 deg` better matches the demo intent. Next gate: run `ros_s15_elbow_curl_demo.py` as a slow `J1 -2 deg` / `J4 +2 deg` arm-only probe, then execute the formal `J1 -10 deg` / `J4 +10 deg` hand open/close/open demo only after the probe is accepted. |

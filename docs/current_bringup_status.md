...
| S13 低风险双臂协同原语 | Corrected direction-sign dry-run accepted; execution pending | Original Arm A `joint1 +30 deg` / Arm B `joint1 -30 deg` execution was numerically healthy but failed operator-visible direction semantics. Post-motion snapshot `20260626_090214` is clean. Corrected dry-run Arm A `joint1 +30 deg` / Arm B `joint1 +30 deg` changes only each arm's `joint1`; A target `1.110 -> 31.110 deg`, B target `-1.988 -> 28.012 deg`; A/B `err_status: 0`; hold max deviation `arm_a: 0.008 deg`, `arm_b: 0.006 deg`. Next gate: corrected execution after safety confirmation. |

...
| S15 双臂双手协调脚本 | Left-side dry-run accepted; left execute gated | New scripts `launch_s15_dual_arm_hand_observe.sh` and `ros_s15_arm_hand_sequence.py` provide RViz observation, selectable read-only/active ROS arm drivers, static TF, segmented arm motion, LinkerHand SDK hand open/close/open, conservative posture guards, dry-run default, and explicit execution confirmations. On 2026-07-02 raw RViz showed horizontal arms while the real arms were hanging; the anchored RViz visual path was added and operator accepted visual pose/follow. The `--side left` dry-run maps to Arm B + left hand `can1`, generated 9 waypoints, and passed as dry-run with max planned joint delta `89.868 deg`. It is a wide-motion plus full-fist plan, so execute remains blocked until the operator confirms Arm B sweep clearance, left-hand closing clearance, RViz visibility, and cable slack. |

...
| S12 控制隔离与日志闭环 | S12.2 Arm B dry-run accepted; execution pending | S12.1 Arm A is accepted and closed. S12.2 Arm B `joint1 -30 deg` dry-run changed only Arm B `joint1` from `-1.988 deg` to `-31.988 deg`; Arm A was passive read-only. Both target and passive statuses had `err_status: 0`. |
